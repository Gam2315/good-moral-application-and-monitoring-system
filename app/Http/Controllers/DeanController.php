...
class DeanController extends Controller
{
  ...
  public function dashboard(Request $request)
  {
    ...
    $minorpending = $this->applyDateFilter(StudentViolation::where('status', '!=', 2)->where('offense_type', 'minor')->where('department', $department), $frequency)->count();
    $minorcomplied = $this->applyDateFilter(StudentViolation::where('status', '=', 2)->where('offense_type', 'minor')->where('department', $department), $frequency)->count();
    $majorpending = $this->applyDateFilter(StudentViolation::where('status', '!=', 2)->where('offense_type', 'major')->where('department', $department), $frequency)->count();
    $majorcomplied = $this->applyDateFilter(StudentViolation::where('status', '=', 2)->where('offense_type', 'major')->where('department', $department), $frequency)->count();
    ...
    $allViolationsByProgram = $this->applyDateFilter(StudentViolation::query()->where('department', $department), $frequency)
