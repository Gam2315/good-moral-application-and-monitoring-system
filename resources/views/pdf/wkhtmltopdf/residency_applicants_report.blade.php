...
<body>
    ...
    <div class="report-page">
        ...
        <div style="text-align: center; margin: 30px 0;">
            <div style="font-size: 16pt; font-weight: bold; margin-bottom: 5px;">APPLICATION OF CERTIFICATE OF RESIDENCY</div>
            @if(isset($time_period_info) && $time_period !== 'all')
                <div style="font-size: 14pt; margin-bottom: 5px;">{{ $time_period_info['title'] }}</div>
                <div style="font-size: 13pt; margin-bottom: 15px;">{{ $time_period_info['period'] }}</div>
            @else
                <div style="font-size: 14pt; margin-bottom: 15px;">A.Y. {{ $academic_year }}</div>
            @endif
            <div style="font-size: 14pt; font-weight: bold; text-decoration: underline;">{{ strtoupper(date('F Y', strtotime($generated_date))) }}</div>
        </div>
